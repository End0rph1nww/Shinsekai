...
class RegistryPluginRecord:
    ...
    @property
    def normalized_support_platforms(self) -> list[str]:
        return list(self.support_platforms or [])
    ...
    for i, (key, item) in enumerate(_registry_items(raw)):
        ...
        out.append(
            RegistryPluginRecord(
                ...
                support_platforms=_as_string_list(
                    item.get("support_platforms")
                    if item.get("support_platforms") is not None
                    else item.get("supportPlatforms")
                ),
